...
const pStory = read('js/systems/camp-p-story.js')
if (!/S 路线 · 商团执行人（开发中）[\s\S]{0,100}disabled: true/.test(pStory)) throw new Error('S route must be visibly disabled')
if (!/M 路线 · 受支配者/.test(pStory) || !/自由身路线 · 受监视的旅人/.test(pStory)) throw new Error('M and free routes must remain selectable and labeled')
...
if (!/visitBanditCamp/.test(read('js/systems/commission.js'))) throw new Error('Bandit camp needs a dedicated persistent map entry')
